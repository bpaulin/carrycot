...
PASSWORD=$(/usr/bin/openssl passwd -crypt "$RAW_PASSWORD")
...
cat <<-EOF > "${TARGET_DIR}${CONFIG_SCRIPT}"
	...
	/usr/bin/useradd --password ${PASSWORD}  --create-home --user-group ${RAW_USERNAME}
	echo 'Defaults env_keep += "SSH_AUTH_SOCK"' > /etc/sudoers.d/10_vagrant
	echo '${RAW_USERNAME} ALL=(ALL) NOPASSWD: ALL' >> /etc/sudoers.d/10_vagrant
EOF
